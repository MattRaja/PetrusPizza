...
namespace BLL.App.Services
{
    public class PizzaSizeService :
        BaseEntityService<IAppUnitOfWork, IPizzaSizeRepository, IPizzaSizeServiceMapper, DAL.App.DTO.PizzaSize,
            PizzaSize>, IPizzaSizeService
    {
        public PizzaSizeService(IAppUnitOfWork uow) : base(uow, uow.PizzaSizes, new PizzaSizeServiceMapper())
        {
        }


        // public virtual async Task<IEnumerable<PizzaSizeView>> GetAllForViewAsync(int minLocationsCount = 10, double minDuration = 60, double minDistance = 10, DateTime? fromDateTime = null, DateTime? toDateTime = null)
        // {
        //     return (await Repository.GetAllForViewAsync(minLocationsCount, minDuration, minDistance, fromDateTime, toDateTime)).Select(e => Mapper.MapPizzaSizeView(e));
        // }
        //
        // public async Task UpdateStatisticsAsync(Guid id)
        // {
        //     // take the session, with all the locations. recalculate stats.
        //     var session = await  UOW.PizzaSizes.GetFirstWithAllLocationsAsync(id);
        ...
        //
        //     await UOW.PizzaSizes.UpdateAsync(session);
        ...
        // }
        public Task<IEnumerable<PizzaSize>> GetAllAsync(Guid appUserId, Guid? userId = null, bool noTracking = true)
        {
            throw new NotImplementedException();
        }

        public async Task AddAndUpdateAsync(PizzaSize pizzaSize)
        {
            var mapper = new PizzaSizeServiceMapper();
            var dalPizzaSize = mapper.MapPizzaSizeBLLDTO(pizzaSize);
            UOW.PizzaSizes.Add(dalPizzaSize);
            await UOW.SaveChangesAsync();
        }
    }
}
