...
namespace BLL.App.Services
{
    public class PizzaTypeService :
        BaseEntityService<IAppUnitOfWork, IPizzaTypeRepository, IPizzaTypeServiceMapper, DAL.App.DTO.PizzaType,
            PizzaType>, IPizzaTypeService
    {
        ...
        // public virtual async Task<IEnumerable<PizzaTypeView>> GetAllForViewAsync(int minLocationsCount = 10, double minDuration = 60, double minDistance = 10, DateTime? fromDateTime = null, DateTime? toDateTime = null)
        // {
        //     return (await Repository.GetAllForViewAsync(minLocationsCount, minDuration, minDistance, fromDateTime, toDateTime)).Select(e => Mapper.MapPizzaTypeView(e));
        // }
        //
        // public async Task UpdateStatisticsAsync(Guid id)
        // {
        //     // take the session, with all the locations. recalculate stats.
        //     var session = await  UOW.PizzaTypes.GetFirstWithAllLocationsAsync(id);
        ...
        //
        //     await UOW.PizzaTypes.UpdateAsync(session);
        ...
        // }
        public Task<IEnumerable<PizzaType>> GetAllAsync(Guid appUserId, Guid? userId = null, bool noTracking = true)
        {
            throw new NotImplementedException();
        }

        public async Task AddAndUpdateAsync(PizzaType pizzaType)
        {
            var mapper = new PizzaTypeServiceMapper();
            var dalPizzaType = mapper.MapPizzaTypeBLLDTO(pizzaType);
            UOW.PizzaTypes.Add(dalPizzaType);
            await UOW.SaveChangesAsync();
        }
    }
}
