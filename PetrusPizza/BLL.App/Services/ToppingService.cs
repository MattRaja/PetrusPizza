...
namespace BLL.App.Services
{
    public class ToppingService :
        BaseEntityService<IAppUnitOfWork, IToppingRepository, IToppingServiceMapper, DAL.App.DTO.Topping,
            Topping>, IToppingService
    {
        ...
        public virtual async Task<IEnumerable<Topping>> GetAllAsync()
        {
            return (await Repository.GetAllAsync()).Select(e => Mapper.MapToppingView(e));
        }
        
        // public async Task UpdateStatisticsAsync(Guid id)
        // {
        //     // take the session, with all the locations. recalculate stats.
        //     var session = await  UOW.Toppings.GetFirstWithAllLocationsAsync(id);
        ...
        //
        //     await UOW.Toppings.UpdateAsync(session);
        ...
        // }
        public Task<IEnumerable<Topping>> GetAllAsync(Guid appUserId, Guid? userId = null, bool noTracking = true)
        {
            throw new NotImplementedException();
        }

        public async Task AddAndUpdateAsync(Topping topping)
        {
            var mapper = new ToppingServiceMapper();
            var dalTopping = mapper.MapToppingBLLDTO(topping);
            UOW.Toppings.Add(dalTopping);
            await UOW.SaveChangesAsync();
        }
    }
}
